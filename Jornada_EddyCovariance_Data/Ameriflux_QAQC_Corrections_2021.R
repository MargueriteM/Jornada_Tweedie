...
flux.21 <- fread("US-Jo1_HH_202101010000_202201010000_submit.csv",sep=",", dec=".",
                 header = TRUE, na.strings=c("na","NA","","-9999"))

flux.21 <- flux.21[,date_time := parse_date_time(TIMESTAMP_END,"YmdHM",tz="UTC")]

# remove duplicate data
flux.21 <- (flux.21[!(duplicated(flux.21, by=c("TIMESTAMP_START")))])

# variables to check from Ameriflux QA/QC
# 2020: PPFD_IN_1_1_1, CO2, G_1_1_1, G_1_2_1, SW_IN_1_1_1, SWC probes.

# Check PPFD_IN_1_1_1
# Decision: APPROVE. some values high but are in line with pattern & only very few during peak summer
ggplot(flux.21, aes(date_time, PPFD_IN_1_1_1))+
  geom_point(size=0.5)+
...
p.co2 <- ggplot(flux.21, aes(date_time, CO2))+
  geom_point(size=0.5)

p.rain <- ggplot(flux.21, aes(date_time, P_RAIN_1_1_1))+
  geom_point(size=0.5)
...
ggplot(flux.21, aes(hour(date_time), CO2))+
  geom_point(size=0.5)+
  facet_wrap(month(date_time)~.)

# diunral rain
ggplot(flux.21, aes(hour(date_time), P_RAIN_1_1_1))+
  geom_point(size=0.5)+
  facet_wrap(month(date_time)~.)

# graph CO2 when fluxes are removed or not removed
ggplot(flux.21[is.na(FC),], aes(date_time, CO2))+
      geom_point(size=0.5)
...
# Decision: APPROVE. largest out-of-range during rain events
ggplot(flux.21, aes(x=date_time))+
...
# look at dirunal
ggplot(flux.21, aes(x=hour(date_time)))+
