...
############################################
#  Process full output files from Eddy Pro #
#       for closed path analysis.          #
...
flux <- fread("/Users/memauritz/Library/CloudStorage/OneDrive-UniversityofTexasatElPaso/Bahada/Tower/ts_data_2/2024/EddyPro_Out/ClosedPath/eddypro_JER_2024_JanMay_Closed_full_output_2024-06-11T182817_exp.csv",
                  sep=",", skip=3,header=FALSE, na.strings=c("-9999","-9999.0","NAN","#NAME?"),
              col.names=colnames(flux.units))
...
flux[,':=' (date=as.Date(date),
            date_time = as.POSIXct(date_time, format=c("%Y-%m-%d %H:%M")),
            year=year(date_time))]
...
# For some reason 2024-03-10 02:00 and 02:30 will not convert to posixct
# drop them. Can't figure out why
flux <- flux[!is.na(date_time)]
...
# graph agc_mean. For closed path, AGC is signal strength. 
...
# remove AGC (signal strengtj) less than 50
flux[agc_mean <50, ':=' (filter_fc = 1L,
                                 filter_LE = 1L,
                                 filter_H = 1L)]
...
# For closed path, don't remove rain
...
flux[DOY<195&year==2023, filter_fc_roll := 0L]
