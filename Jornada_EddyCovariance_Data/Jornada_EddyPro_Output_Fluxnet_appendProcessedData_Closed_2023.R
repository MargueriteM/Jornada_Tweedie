...
# closed path from 2023
 flux_add <- fread("/Users/memauritz/Library/CloudStorage/OneDrive-UniversityofTexasatElPaso/Bahada/Tower/ts_data_2/2023/EddyPro_Out/ClosedPath/eddypro_JER_2023_JulDec_Closed_fluxnet_2024-06-10T104234_exp.csv",
...
biomet2023.names <- colnames(fread("/Users/memauritz/Library/CloudStorage/OneDrive-UniversityofTexasatElPaso/Bahada/Tower/EddyCovariance_ts/EddyPro_Biomet/Biomet_EddyPro_2023.csv",
                                   header=TRUE))
...
biomet2023 <- fread("/Users/memauritz/Library/CloudStorage/OneDrive-UniversityofTexasatElPaso/Bahada/Tower/EddyCovariance_ts/EddyPro_Biomet/Biomet_EddyPro_2023.csv",
                    skip=2, header=FALSE, col.names=biomet2023.names, na.strings=c("-9999"))


biomet2023 <- biomet2023[,':='
                         (date_time=ymd_hm(paste(timestamp_1,timestamp_2,timestamp_3,timestamp_4,timestamp_5, sep=" ")))]

ggplot(biomet2023[date_time>as.Date("2023-07-05")], aes(date_time,P_rain_1_1_1))+
  geom_line()
...
flux_add[DOY_START<195, filter_fc_roll := 0L]
...
# keep the first 3 days
flux_add[(DOY_START<195&NIGHT==0) | (DOY_START<196&NIGHT==1), filter_fc_roll_daynight := 0L]


# view the marked fluxes in ~25 day chunks
ggplot(flux_add[filter_fc_roll!=1&DOY_START>=180&DOY_START<=366,], aes(DOY_START, FC))+
  geom_line()+
  geom_point(aes(colour=factor(filter_fc_roll)), size=0.5)

# view the marked fluxes in ~25 day chunks for day/night
ggplot(flux_add[filter_fc_roll_daynight!=1&DOY_START>=180&DOY_START<=366,], aes(DOY_START, FC))+
  geom_line()+
...
# keep the first 3 days
flux_add[DOY_START<195, filter_le_roll := 0L]
flux_add[(DOY_START<195&LE>200 | DOY_START<195&LE<(-200)), filter_le_roll := 2L]
...
# keep the first 3 days
flux_add[(DOY_START<195&NIGHT==0) | (DOY_START<196&NIGHT==1), filter_le_roll_daynight := 0L]
flux_add[(DOY_START<195&LE>200) | (DOY_START<195&LE<(-200)), filter_le_roll_daynight := 2L]

# view the marked flux_addes in ~25 day chunks
ggplot(flux_add[filter_le_roll!=1&DOY_START>=180&DOY_START<=366,], aes(DOY_START, LE, colour=factor(filter_le_roll)))+
  geom_point()

# view the marked flux_addes in ~25 day chunks for day/night
ggplot(flux_add[filter_le_roll_daynight!=1&DOY_START>=180&DOY_START<=366,], aes(DOY_START, LE))+
  geom_line()+
...
# keep first 3 days
flux_add[DOY_START<195, filter_h_roll := 0L]
flux_add[DOY_START<195&H<(-100), filter_h_roll := 2L]
...
# keep first 3 days
flux_add[DOY_START<195, filter_h_roll_daynight := 0L]
flux_add[DOY_START<195&H<(-100), filter_h_roll_daynight := 2L]

# view the marked flux_addes in ~25 day chunks
ggplot(flux_add[filter_h_roll!=1&DOY_START>=180&DOY_START<=366,], aes(DOY_START, H))+
  geom_line()+
  geom_point(aes(colour=factor(filter_h_roll)), size=0.5)

# view the marked flux_addes in ~25 day chunks for day/night
ggplot(flux_add[filter_h_roll_daynight!=1&DOY_START>=180&DOY_START<=366,], aes(DOY_START, H))+
  geom_line()+
...
flux_filter_sd_all <- flux_add_filter_sd
...
write.table(flux_add_filter_sd,
            file="JER_flux_2023_EddyPro_Output_filtered_SD_JuneDec_Closed.csv",sep=",", dec=".",
            row.names=FALSE)
